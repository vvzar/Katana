...
class AutoUpdater{
	...
	public function showConsoleUpdate(){
		...
		$logger->warning("----- Katana Auto Updater -----");
		$logger->warning("Your version of Katana is out of date. Version " . $newVersion->get(false) . " (build #" . $newVersion->getBuild() . ") was released on " . date("D M j h:i:s Y", $this->updateInfo["date"]));
	...
	public function showPlayerUpdate(Player $player){
		$player->sendMessage(TextFormat::DARK_PURPLE . "The version of Katana that this server is running is out of date. Please consider updating to the latest version.");
	...
	protected function showChannelSuggestionStable(){
		$logger = $this->server->getLogger();
		$logger->info("----- Katana Auto Updater -----");
